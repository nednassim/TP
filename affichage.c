...
void Afficher_Perso(Tenreg personnel, int i) {
	...
	printf("*| ---------------%*s\n", 48, "|*");
	printf("*| Matricule : %d %*s \n", personnel.matricule, 44, "|*");
	int len1 = 62 - strlen(personnel.nom) - 6;
	printf("*| Nom : %s %*s\n", personnel.nom, len1, "|*");
	int len2 = 62 - strlen(personnel.prenom) - 9;
	printf("*| Prenom : %s %*s\n", personnel.prenom, len2, "|*");
	printf("*| Date de naissance : %s %*s\n", personnel.date_naissance, 32, "|*");
	int len3 = 62 - strlen(wilayas[personnel.wilaya_naissance - 1]) - 22;
	printf("*| Wilaya de naissance : %s %*s\n", wilayas[personnel.wilaya_naissance - 1], len3, "|*");
	int len4 = 62 - strlen(groupe_sanguins[personnel.groupe_sanguin - 1]) - 17;
	printf("*| Groupe sanguin : %s %*s\n", groupe_sanguins[personnel.groupe_sanguin - 1], len4, "|*");
	int len5 = 62 - strlen(grades[personnel.grade - 1]) - 8;
	printf("*| Grade : %s %*s\n", grades[personnel.grade - 1], len5, "|*");
	int len6 = 62 - strlen(force_armees[personnel.force_armee - 1]) - 14;
	printf("*| Force armee : %s %*s\n", force_armees[personnel.force_armee - 1], len6, "|*");
	int len7 = 62 - strlen(region_militaires[personnel.region_militaire - 1]) - 19;
	printf("*| Region militaire : %s %*s\n", region_militaires[personnel.region_militaire - 1], len7, "|*");
